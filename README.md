...
## Project Overview
This repository documents my HW4 project for **DATA 6545**, which extends the Olist customer satisfaction prediction problem into a broader MLOps workflow. The project moves beyond training a single machine learning model and instead addresses several practical production oriented tasks, including foundation model evaluation, experiment tracking, model serving, monitoring, containerization, cloud deployment, and documentation.
...
## Repository Structure
    ...
    ├── .dockerignore
    ├── Dockerfile
    ├── HW4_full_workflow_notebook_clean.ipynb
    ├── README.md
    ├── app.py
...
to a **500 record sample** of Olist review text with non empty `review_comment_message` values. The model predicts 1 to 5 stars from the review text, and those predictions were mapped into the binary customer satisfaction target used throughout the project:

* 4 to 5 stars become positive `1`
* 1 to 3 stars become negative `0`
...
4. missing required field returns 400  
...
## Part 4 Docker and Cloud Deployment
Part 4 extends the local API into a production oriented deployment workflow.

### Containerization
The repository includes deployment support through:
* `Dockerfile`
* `.dockerignore`

The deployment workflow uses the project Dockerfile and GitHub repository structure to package the API and model artifacts for production.

### Cloud deployment
The API was deployed to **Render** as a web service connected to this GitHub repository.

### Live deployed API URL
* `https://hw4-mlops-api.onrender.com`

### Live endpoint checks
The deployed API was tested successfully using:
...
### Supporting screenshots
Stored in:
* `screenshots/HW4_render_live_health.png`
* `screenshots/HW4_render_live_predict.png`
...
* **Months 1 to 3:** stable period with small random variation
* **Months 4 to 6:** progressively drifted period
...
* `https://hw4-mlops-api.onrender.com/health`

Because the root `/` route is not defined in the Flask app, visiting the base URL in a browser may return **Not Found**. This is expected behavior. The main deployed checks should be performed through the documented endpoints such as `/health` and `/predict`.
...
* live Render health response
* live Render prediction response
